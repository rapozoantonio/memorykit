...
- After the `sharp` fix, `onnxruntime-node`'s own internal binding probe (a separate native dependency, also transitive via `@xenova/transformers`) surfaced an unhandled promise rejection on `ubuntu-latest` (`Module did not self-register` on `onnxruntime_binding.node`) — non-fatal, but it failed the CI test run despite all 204 tests passing. Embedding generation itself was unaffected (already degrades gracefully per the dynamic-import fix above). Added a `process.on("unhandledRejection", ...)` handler in both `server.ts` (production) and a new `src/__tests__/setup.ts` (test runs) to log and continue rather than fail. Considered upgrading `onnxruntime-node` directly instead, but versions `1.16.3` and `1.26.0` both broke `@xenova/transformers@2.17.2`'s expected API in different ways (`no available backend found`, `Tensor.location must be a string`) — reverted those attempts; the exact upstream-pinned `1.14.0` is correct, it just needs this one rejection handled.
