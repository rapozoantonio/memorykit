...
export class ProcessManager {
  ...
  constructor(config: ProcessManagerConfig) {
    ...
    this.externalApi = config.externalApi ?? false;
    this.useDocker = config.useDocker ?? true; // Default to Docker
    this.dockerComposeFile =
      config.dockerComposeFile || this.getDefaultDockerComposePath();
  ...
  async stop(): Promise<void> {
    if (this.externalApi) {
      console.error("[ProcessManager] External API - nothing to stop");
    } else if (this.useDocker) {
      await this.stopDocker();
    } else {
  ...
  private async stopDocker(): Promise<void> {
    ...
    const stopProcess = spawn(
      "docker-compose",
      ["-f", this.dockerComposeFile, "--profile", "mcp", "down"],
      {
        stdio: "inherit",
        shell: true,
      }
